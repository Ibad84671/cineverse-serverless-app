...
TEMPLATE="$ROOT/infrastructure/cineverse.yaml"
...
aws cloudformation deploy \
  ...
  --parameter-overrides ProjectName="$PROJECT_NAME" EnvironmentName="$ENVIRONMENT" CognitoDomainPrefix="$DOMAIN_PREFIX"
...
DIST_DOMAIN="$(aws cloudformation describe-stacks --stack-name "$STACK_NAME" --region "$REGION" --query "Stacks[0].Outputs[?OutputKey=='CloudFrontDomainName'].OutputValue" --output text)"
