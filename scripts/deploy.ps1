...
$Template = Join-Path $Root 'infrastructure\cineverse.yaml'
...
aws cloudformation deploy `
  ...
  --parameter-overrides "ProjectName=$ProjectName" "EnvironmentName=$Environment" "CognitoDomainPrefix=$DomainPrefix"

function Get-Output($key) {
  return (aws cloudformation describe-stacks --stack-name $StackName --region $Region --query "Stacks[0].Outputs[?OutputKey=='$key'].OutputValue" --output text).Trim()
}

$Bucket = Get-Output 'FrontendBucketName'
$ApiUrl = Get-Output 'ApiUrl'
$FrontendUrl = Get-Output 'FrontendUrl'
$CognitoDomain = Get-Output 'CognitoDomain'
$ClientId = Get-Output 'CognitoClientId'
$DistDomain = Get-Output 'CloudFrontDomainName'
